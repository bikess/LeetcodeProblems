...
//    	把rusult的重复的取掉
    ...
    /****
     * 延伸到求ksum，也就是判断在数组A[]（n个数）中是否存在k个数的和为目标值，。
     * 最优的解题思路是这样的：（假设k为偶数,以4为例）
     * 先求出任意k/2（2个）数的和(求数组中任意2个数的和的个数为n^2(n^(k/2))，然后把任意的2/k个数存到hashSet中去，然后遍历一遍这n^2个数字，然后在hashset中寻找是否存在target-当前的数
     * 这个复杂度为0（n^2),所以最后的时间复杂度还是n^2,一定要注意这里无需进行排列了，有时候利用hashSet求解非常方便，一定时刻想着用hashset，
     * 当判断某个数是否存在某个数组中的时候，利用hashset可以的时间复杂度为0（1），而用折半查找时0（lgn）
     * 
     * 因此寻找kSum的，也就是判断在数组A[]（n个数）中是否存在k个数的和为目标值的最优复杂度是：0(n^(k/2))(这是枚举任意k/2个数的和，把结果存在hashset当中)+o(n^(k/2))（利用hashset判断是否存在target-某一个k/2个数的和）= o(n^(k/2))
     */
